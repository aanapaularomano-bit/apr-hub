...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await sb
    ...
    .select('id, from_who, title, status, note, created_at')
...
export async function POST(req: NextRequest) {
  ...
  const session = await auth(req, slug);
  if (!session) return NextResponse.json({ error: 'Não autenticado' }, { status: 401 });
  const { from_who, title, status, note } = await req.json();
  if (!title) return NextResponse.json({ error: 'title obrigatório' }, { status: 400 });
  const effectiveFrom = session.role === 'admin' ? (from_who ?? 'agencia') : 'cliente';
  const { data, error } = await sb
    .from('portal_requests')
    .insert({ client_id: session.clientId, from_who: effectiveFrom, title, status: status ?? 'em_andamento', note: note ?? null })
    .select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PUT(req: NextRequest) {
  ...
  const { id, from_who, title, status, note } = await req.json();
  if (!id) return NextResponse.json({ error: 'id obrigatório' }, { status: 400 });
  const { data, error } = await sb
    .from('portal_requests')
    .update({ from_who, title, status, note })
...
export async function DELETE(req: NextRequest) {
  ...
  if (!session || session.role !== 'admin') return NextResponse.json({ error: 'Sem permissão' }, { status: 403 });
  if (!id) return NextResponse.json({ error: 'id obrigatório' }, { status: 400 });
  const { error } = await sb.from('portal_requests').delete().eq('id', id).eq('client_id', session.clientId);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
