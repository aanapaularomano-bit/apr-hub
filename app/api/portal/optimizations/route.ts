...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await sb
    ...
    .select('id, date, type, campaign, action, reason, result, created_at')
...
export async function POST(req: NextRequest) {
  ...
  const { date, type, campaign, action, reason, result } = await req.json();
  if (!date || !type || !action) return NextResponse.json({ error: 'date, type e action obrigatórios' }, { status: 400 });
  const { data, error } = await sb
    .from('portal_optimizations')
    .insert({ client_id: session.clientId, date, type, campaign: campaign ?? null, action, reason: reason ?? null, result: result ?? 'obs' })
    .select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PUT(req: NextRequest) {
  ...
  const session = await auth(req, slug);
  if (!session || session.role !== 'admin') return NextResponse.json({ error: 'Sem permissão' }, { status: 403 });
  const { id, date, type, campaign, action, reason, result } = await req.json();
  if (!id) return NextResponse.json({ error: 'id obrigatório' }, { status: 400 });
  const { data, error } = await sb
    .from('portal_optimizations')
    .update({ date, type, campaign, action, reason, result })
...
export async function DELETE(req: NextRequest) {
  ...
  if (!session || session.role !== 'admin') return NextResponse.json({ error: 'Sem permissão' }, { status: 403 });
  if (!id) return NextResponse.json({ error: 'id obrigatório' }, { status: 400 });
  const { error } = await sb.from('portal_optimizations').delete().eq('id', id).eq('client_id', session.clientId);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
