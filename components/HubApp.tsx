...
Responda a pergunta da Ana Paula sobre a agência.`;
  ...
  if (page === 'dashboards') {
    ...
    async function createDash() {
      ...
      const { data } = await supabase.from('client_dashboards').insert({ ...dForm, dash_code: code, template: 'simple', user_id: user.id, daily_data: [], ads_data: [] }).select().single();
      if (data) { setDashes([data, ...dashes]); setEditDash(data); }
      setShowNewDash(false); setDForm({});
    }
    ...
    async function deleteDash(id: string) {
      ...
      setDashes(dashes.filter(d => d.id !== id)); setEditDash(null);
    }
    ...
    if (editDash) {
      ...
      return (<div style={{ minHeight: '100vh', background: T.bg, color: T.tx, fontFamily: T.fn, display: 'flex' }}>{sidebar}<main style={{ flex: 1, padding: '24px 28px', overflowY: 'auto', maxHeight: '100vh' }}>
        ...
        <div style={{ background: T.card, border: '1px solid ' + T.bdr, borderRadius: 14, padding: 22, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
            {[['leads_total', 'Leads Total'], ['cpl', 'CPL (R$)'], ['investimento', 'Investimento'], ['leads_confirmados', 'Confirmados'], ['taxa_confirmacao', 'Taxa Confirmação %'], ['leads_quentes', 'Leads Quentes'], ['leads_frios', 'Leads Frios'], ['leads_whatsapp', 'Leads WhatsApp'], ['grupos_whatsapp', 'Grupos WPP'], ['taxa_aderencia', 'Taxa Aderência %']].map(([k, l]) => (
              <div key={k}><label style={labelS}>{l}</label><input type="number" value={editDash[k] || ''} onChange={e => numUpd(k, e.target.value)} style={inputS} /></div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: T.card, border: '1px solid ' + T.bdr, borderRadius: 14, padding: 22, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
            {[['impressions', 'Impressões'], ['alcance', 'Alcance'], ['cliques', 'Cliques'], ['page_views', 'Page Views'], ['ctr', 'CTR %'], ['cpc', 'CPC (R$)'], ['cpm', 'CPM (R$)'], ['tx_carregamento', 'Tx Carregamento %'], ['tx_conversao', 'Tx Conversão %']].map(([k, l]) => (
              <div key={k}><label style={labelS}>{l}</label><input type="number" value={editDash[k] || ''} onChange={e => numUpd(k, e.target.value)} style={inputS} /></div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: T.card, border: '1px solid ' + T.bdr, borderRadius: 14, padding: 22, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
            {[['faturamento', 'Faturamento'], ['lucro', 'Lucro'], ['vendas_total', 'Vendas Total'], ['taxa_lista', 'Taxa Lista %'], ['cac', 'CAC (R$)'], ['roas', 'ROAS']].map(([k, l]) => (
              <div key={k}><label style={labelS}>{l}</label><input type="number" value={editDash[k] || ''} onChange={e => numUpd(k, e.target.value)} style={inputS} /></div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: T.card, border: '1px solid ' + T.bdr, borderRadius: 14, padding: 22, marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 12, color: T.mt, marginBottom: 8 }}>Formato: [{'"name":"AD01","investimento":176.2,"leads":24,"cpl":7.34,"ctr":0.82,"cpm":15.08'}}, ...]</p>
    ...
    return (<div style={{ minHeight: '100vh', background: T.bg, color: T.tx, fontFamily: T.fn, display: 'flex' }}>{sidebar}<main style={{ flex: 1, padding: '24px 28px', overflowY: 'auto', maxHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button onClick={() => { setDForm({ title: '', client_id: '', theme: 'dark' }); setShowNewDash(true); }} style={btnS('#22c55e')}>+ Novo Dashboard</button>
      </div>

      {dLoading ? <div style={{ padding: 40, textAlign: 'center', color: T.mt }}>Carregando...</div> :
        dashes.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 60, color: T.mt }}>
            ...
            <button onClick={() => { setDForm({ title: '', client_id: '', theme: 'dark' }); setShowNewDash(true); }} style={btnS('#6366f1')}>Criar primeiro dashboard</button>
          </div>
        ) : dashes.map(d => {
            ...
            <div key={d.id} style={{ background: T.card, border: '1px solid ' + T.bdr, borderRadius: 14, padding: '16px 20px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 5, background: d.theme === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(245,158,11,0.1)', color: d.theme === 'dark' ? T.mt : '#f59e0b' }}>{d.theme === 'dark' ? '🌙' : '☀️'} {d.template}</span>
              ...
              <a href={link} target="_blank" rel="noopener noreferrer" style={{ ...btnS('#22c55e'), fontSize: 12, padding: '8px 14px', textDecoration: 'none' }}>👁️ Ver</a>
      ...
      {/* New dashboard modal */}
      {showNewDash && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setShowNewDash(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#12121e', border: '1px solid ' + T.bdr, borderRadius: 16, padding: 28, width: 440 }}>
            ...
            <div style={{ marginBottom: 12 }}><label style={labelS}>Título</label><input value={dForm.title || ''} onChange={e => setDForm({ ...dForm, title: e.target.value })} placeholder="Ex: Captação Lançamento Maio" style={inputS} /></div>
            <div style={{ marginBottom: 12 }}><label style={labelS}>Cliente</label><select value={dForm.client_id || ''} onChange={e => setDForm({ ...dForm, client_id: e.target.value })} style={inputS}><option value="">Selecionar...</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
            <div style={{ marginBottom: 16 }}><label style={labelS}>Tema</label><div style={{ display: 'flex', gap: 4 }}>{['dark', 'light'].map(t => <button key={t} onClick={() => setDForm({ ...dForm, theme: t })} style={{ flex: 1, padding: 10, borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600, background: dForm.theme === t ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.03)', border: dForm.theme === t ? '1px solid rgba(99,102,241,0.3)' : '1px solid ' + T.bdr, color: dForm.theme === t ? '#a5b4fc' : T.mt }}>{t === 'dark' ? '🌙 Dark' : '☀️ Light'}</button>)}</div></div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowNewDash(false)} style={{ flex: 1, padding: 12, borderRadius: 10, border: '1px solid ' + T.bdr, background: 'transparent', color: T.mt, cursor: 'pointer', fontSize: 14 }}>Cancelar</button>
              <button onClick={createDash} disabled={!dForm.title} style={{ flex: 1, padding: 12, borderRadius: 10, border: 'none', background: dForm.title ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : 'rgba(255,255,255,0.05)', color: dForm.title ? '#fff' : T.mt, cursor: dForm.title ? 'pointer' : 'not-allowed', fontSize: 14, fontWeight: 700 }}>Criar</button>
            </div>
          </div>
        </div>
      )}
